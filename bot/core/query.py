...
tapper_semaphore = asyncio.Semaphore(5)  # Giới hạn 5 Tapper cùng lúc, bạn có thể điều chỉnh số này
...
async def run_multiple_tappers(queries, session_names, proxies):
    while True:
        tappers = [Tapper(query, name, True) 
                   for query, name in zip(queries, session_names)]
        ...
        sleep_ = random.randint(2600, 3100)
        logger.info(f"<red>Tất cả Tapper đã hoàn thành. Nghỉ {sleep_}s trước khi khởi động lại...</red>")
        await asyncio.sleep(sleep_)

# Sử dụng hàm này trong main
# async def main():
#     queries = ["query1", "query2", "query3", "query4", "query5", "query6", "query7"]
#     session_names = ["session1", "session2", "session3", "session4", "session5", "session6", "session7"]
#     proxies = ["proxy1", "proxy2", "proxy3"]
#     await run_multiple_tappers(queries, session_names, proxies, max_concurrent_threads=5)
